Feature: Product
  ...
  Scenario Outline: POST Product Twice with same code - expect <responseCode> as response code
    ...
    * def count = db.readValue('select count(*) FROM item.product')
    ...
    * method POST
    * status <responseCode>
    * eval if(__num==1 && count != db.readValue('select count(*) FROM item.product')) karate.fail("product count values in DB are different")
